...
def test_is2_outer_uses_callable_and_returns_value():
    ...
    got = compute_is2_outer_bridge_energy("ACGU", t, 0, 5, 1, 4)
...
def test_is2_outer_defaults_to_zero_when_missing_or_none():
    ...
    # tables is None
    assert compute_is2_outer_bridge_energy("ACGU", None, 0, 5, 1, 4) == 0.0
    # attribute missing
    assert compute_is2_outer_bridge_energy("ACGU", TablesWithoutAttr(), 0, 5, 1, 4) == 0.0
...
def test_is2_outer_yhx_uses_callable_and_casts_to_float():
    ...
    got = compute_is2_outer_bridge_energy_yhx(Cfg(), "ACGU", 0, 5, 1, 4)
...
def test_is2_outer_yhx_defaults_to_zero_when_no_tables_or_missing_attr():
    ...
    # tables is None
    assert compute_is2_outer_bridge_energy_yhx(CfgNoTables(), "ACGU", 0, 5, 1, 4) == 0.0
    # IS2_outer_yhx attribute missing
    assert compute_is2_outer_bridge_energy_yhx(CfgWithTablesNoAttr(), "ACGU", 0, 5, 1, 4) == 0.0
...
def test_is2_outer_yhx_raises_if_attribute_is_non_callable():
    ...
    with pytest.raises(TypeError):
        compute_is2_outer_bridge_energy_yhx(Cfg(), "ACGU", 0, 5, 1, 4)
