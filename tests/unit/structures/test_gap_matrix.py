...
def test_sparse_gap_matrix_defaults_and_bounds():
    ...
    """
    n = 6
    matrix = SparseGapEnergyMatrix(seq_len=n)

    # An unset cell with valid coordinates should return the default value of +infinity.
    assert math.isinf(matrix.get_energy(0, 0, 0, 0))
    assert math.isinf(matrix.get_energy(1, 4, 2, 3))

    # --- Test boundary checks and index constraints ---
    # The matrix should return +infinity if any index violates the required ordering:
    # 0 <= i <= k < l <= j < n
    assert math.isinf(matrix.get_energy(-1, 2, 0, 1))  # i < 0
    assert math.isinf(matrix.get_energy(0, n, 0, 1))   # j >= n
    assert math.isinf(matrix.get_energy(3, 2, 3, 3))   # i > j
    assert math.isinf(matrix.get_energy(2, 5, 1, 4))   # k < i
    assert math.isinf(matrix.get_energy(1, 4, 1, 5))   # l > j
    assert math.isinf(matrix.get_energy(1, 4, 3, 2))   # k > l
...
def test_sparse_gap_matrix_set_get_roundtrip_and_overwrite():
    ...
    matrix.set_energy(1, 5, 2, 4, -3.25)
    assert matrix.get_energy(1, 5, 2, 4) == -3.25

    # Because the matrix is sparse, setting one "hole" (k,l) within an outer
    # span (i,j) should not affect other holes in the same span.
    assert math.isinf(matrix.get_energy(1, 5, 2, 3))

    # Verify that overwriting the same cell works as expected.
    matrix.set_energy(1, 5, 2, 4, -7.0)
    assert matrix.get_energy(1, 5, 2, 4) == -7.0
...
def test_sparse_gap_matrix_row_is_live_view():
    ...
    row = matrix.get_outer_span_map(1, 3)
    ...
    assert matrix.get_energy(1, 3, 2, 3) == -1.23

    # A second mutation should also be reflected.
    row[(1, 3)] = -0.5
    assert matrix.get_energy(1, 3, 1, 3) == -0.5
...
def test_sparse_gap_matrix_no_collapse_identity_inside_class():
    ...
    matrix.set_energy(1, 4, 2, 3, -0.75)
    assert matrix.get_energy(1, 4, 2, 3) == -0.75
...
def test_sparse_gap_backptr_defaults_and_roundtrip():
    ...
    """
    n = 6
    back_ptr = SparseGapBackpointerMatrix(seq_len=n)

    # An unset backpointer should default to None.
    assert back_ptr.get_backpointer(1, 4, 2, 3) is None

    # Perform a set/get round-trip with a sample backpointer object.
    obj = {"op": "WHX_SPLIT", "args": (2,)}
    back_ptr.set_backpointer(1, 4, 2, 3, obj)
    assert back_ptr.get_backpointer(1, 4, 2, 3) is obj

    # Verify that overwriting works correctly.
    obj2 = {"op": "WHX_COLLAPSE"}
    back_ptr.set_backpointer(1, 4, 2, 3, obj2)
    assert back_ptr.get_backpointer(1, 4, 2, 3) is obj2
...
def test_sparse_gap_backptr_missing_or_oob_is_none():
    ...
    assert back_ptr.get_backpointer(0, 3, 1, 2) is None
    assert back_ptr.get_backpointer(0, 0, 0, 0) is None

    # Accessing "out-of-bounds" coordinates is equivalent to a missing key.
    assert back_ptr.get_backpointer(-1, 2, 0, 1) is None
    assert back_ptr.get_backpointer(0, 5, 1, 2) is None
    assert back_ptr.get_backpointer(2, 1, 1, 2) is None
