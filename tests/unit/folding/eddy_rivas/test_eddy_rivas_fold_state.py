...
from rna_pk_fold.structures.gap_matrix import (
    SparseGapEnergyMatrix,
    SparseGapBackpointerMatrix,
)
...
def test_make_state_constructs_all_matrices_and_slots():
    ...
    assert isinstance(st.wx_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.vx_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.wxi_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.wxu_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.wxc_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.vxu_matrix, EddyRivasTriangularEnergyMatrix)
    assert isinstance(st.vxc_matrix, EddyRivasTriangularEnergyMatrix)

    # Verify the types of the corresponding 2D backpointer tables.
    assert isinstance(st.wx_back_ptr, EddyRivasTriangularBackpointerMatrix)
    assert isinstance(st.vx_back_ptr, EddyRivasTriangularBackpointerMatrix)

    # Verify the types of the 4D sparse "gap" matrices for PK components.
    assert isinstance(st.whx_matrix, SparseGapEnergyMatrix)
    assert isinstance(st.vhx_matrix, SparseGapEnergyMatrix)
    assert isinstance(st.yhx_matrix, SparseGapEnergyMatrix)
    assert isinstance(st.zhx_matrix, SparseGapEnergyMatrix)

    # Verify the types of the corresponding 4D backpointer tables.
    assert isinstance(st.whx_back_ptr, SparseGapBackpointerMatrix)
    assert isinstance(st.vhx_back_ptr, SparseGapBackpointerMatrix)
    assert isinstance(st.yhx_back_ptr, SparseGapBackpointerMatrix)
    assert isinstance(st.zhx_back_ptr, SparseGapBackpointerMatrix)
...
def test_non_gap_base_cases_diagonal_and_defaults():
    ...
    for i in range(n):
        ...
        assert st.wx_matrix.get_energy(i, i) == 0.0
        assert st.wxi_matrix.get_energy(i, i) == 0.0
        assert st.wxu_matrix.get_energy(i, i) == 0.0
        assert st.wxc_matrix.get_energy(i, i) == 0.0

        # VX family matrices: a single nucleotide cannot form a pair, so energy is infinite.
        assert math.isinf(st.vx_matrix.get_energy(i, i))
        assert math.isinf(st.vxu_matrix.get_energy(i, i))
        assert math.isinf(st.vxc_matrix.get_energy(i, i))

    # --- Test default values for other cases ---

    # Off-diagonal entries should default to +infinity until they are computed.
    assert math.isinf(st.wx_matrix.get_energy(0, 2))
    assert math.isinf(st.vx_matrix.get_energy(0, 2))

    # Empty subsequences (where j < i) should have an energy of 0.0.
    assert st.wx_matrix.get_energy(1, 0) == 0.0
    assert st.vx_matrix.get_energy(1, 0) == 0.0

    # Accessing out-of-bounds indices should gracefully return +infinity.
    assert math.isinf(st.wx_matrix.get_energy(-1, 0))
    assert math.isinf(st.wx_matrix.get_energy(0, n))
...
def test_non_gap_backpointers_default_none_and_set_get():
    ...
    assert st.wx_back_ptr.get_backpointer(0, 0) is None
    assert st.vx_back_ptr.get_backpointer(0, 2) is None

    # Test a simple set/get round-trip. The payload can be any object.
    st.wx_back_ptr.set_backpointer(0, 2, ("WX_OP", (0, 2)))
    st.vx_back_ptr.set_backpointer(1, 2, ("VX_OP", (1, 2)))

    assert st.wx_back_ptr.get_backpointer(0, 2) == ("WX_OP", (0, 2))
    assert st.vx_back_ptr.get_backpointer(1, 2) == ("VX_OP", (1, 2))
...
def test_gap_matrices_initial_state_and_roundtrip():
    ...
    assert math.isinf(st.whx_matrix.get_energy(0, 5, 2, 4))
    assert math.isinf(st.vhx_matrix.get_energy(1, 4, 2, 3))
    assert math.isinf(st.yhx_matrix.get_energy(0, 3, 1, 2))
    assert math.isinf(st.zhx_matrix.get_energy(2, 5, 3, 4))

    # Corresponding backpointers should default to None.
    assert st.whx_back_ptr.get_backpointer(0, 5, 2, 4) is None
    assert st.vhx_back_ptr.get_backpointer(1, 4, 2, 3) is None

    # Test set/get round-trip for energy values.
    st.whx_matrix.set_energy(0, 5, 2, 4, 7.25)
    st.zhx_matrix.set_energy(2, 5, 3, 4, -1.0)
    assert st.whx_matrix.get_energy(0, 5, 2, 4) == 7.25
    assert st.zhx_matrix.get_energy(2, 5, 3, 4) == -1.0

    # Test set/get round-trip for back-pointers.
    st.yhx_back_ptr.set_backpointer(0, 5, 2, 4, ("YHX_OP", (0, 5, 2, 4)))
    st.zhx_back_ptr.set_backpointer(2, 5, 3, 4, {"op": "ZHX_OP"})
    assert st.yhx_back_ptr.get_backpointer(0, 5, 2, 4) == ("YHX_OP", (0, 5, 2, 4))
    assert st.zhx_back_ptr.get_backpointer(2, 5, 3, 4) == {"op": "ZHX_OP"}
...
def test_setting_values_in_non_gap_matrices_roundtrip():
    ...
    st.wx_matrix.set_energy(0, 3, -3.5)
    st.vx_matrix.set_energy(1, 4, 2.0)

    # Verify that the values can be retrieved correctly.
    assert st.wx_matrix.get_energy(0, 3) == -3.5
    assert st.vx_matrix.get_energy(1, 4) == 2.0
